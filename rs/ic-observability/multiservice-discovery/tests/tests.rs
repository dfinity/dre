...
mod tests {
    ...
    impl TestData {
        fn from_prom(targets: Vec<PrometheusStaticConfig>) -> Self {
            ...
            Self {
                ...
                nodes: labels_set.get(IC_NODE).unwrap()
                .iter().cloned().collect::<Vec<_>>(),
                subnets: labels_set.get(IC_SUBNET).unwrap()
                .iter().cloned().collect::<Vec<_>>(),
            }
        }
    ...
    impl SDRunner {
        async fn fetch_targets(&mut self) -> anyhow::Result<Vec<PrometheusStaticConfig>>  {
            ...
            let mut sd_server = self.command
            .args(args)
            .spawn()
            .unwrap();
        ...
        pub fn from_local_bin() -> Self {
            Self {
                command: Command::cargo_bin(CARGO_SD_BIN)
    ...
    impl ExpectedDataFetcher {
        ...
        async fn from_main_sd(&self) -> anyhow::Result<TestData> {
            ...
            let targets: Vec<PrometheusStaticConfig> = SDRunner::from_remote_bin(MAIN_SD_URL).await.fetch_targets().await?;
        ...
        pub async fn get_expected_data(&self) -> anyhow::Result<TestData> {
            ...
            self.from_public_dashboard_api().await.or(Err(anyhow!("Expected data not found")))
        }
    }
