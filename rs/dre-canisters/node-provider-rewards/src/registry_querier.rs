use std::{collections::{btree_map::Entry, BTreeMap}, rc::Rc, str::FromStr};

use ic_protobuf::registry::{dc::v1::DataCenterRecord, node::v1::NodeRecord, node_operator::v1::NodeOperatorRecord, node_rewards::v2::NodeRewardsTable};
...
use crate::{chrono_utils::DateTimeRange, local_registry::LocalRegistry, types::Node};
...
impl RegistryQuerier {
    pub fn nodes_in_period(&self, period: &DateTimeRange) -> Vec<Node> {
        let mut nodes = BTreeMap::new();
        let mut rewardable_nodes = BTreeMap::new();
        let versions = self.local_registry.registry_versions_in_range(period);

        ic_cdk::println!("versions in range: {:?}", versions);

        for version in versions {
            let registry_version = RegistryVersion::from(version);
            let nodes_in_version = self.local_registry
                .get_family_entries_of_version::<NodeRecord>(NODE_RECORD_KEY_PREFIX, registry_version).unwrap();
