...
const MAX_VALUE_SIZE_BYTES_REWARDS_MULTIPLIER_STATS: u32 = 114;

impl Storable for RewardsMultiplierStats {
    fn to_bytes(&self) -> std::borrow::Cow<[u8]> {
        Cow::Owned(Encode!(self).unwrap())
    }
    ...
    const BOUND: Bound = Bound::Bounded {
        max_size: MAX_VALUE_SIZE_BYTES_REWARDS_MULTIPLIER_STATS,
        is_fixed_size: false,
    };
}
...
const MAX_VALUE_SIZE_BYTES_NODE_PROVIDER_REWARDS: u32 = 37;

impl Storable for NodeProviderRewards {
    fn to_bytes(&self) -> std::borrow::Cow<[u8]> {
        Cow::Owned(Encode!(self).unwrap())
    }
    ...
    const BOUND: Bound = Bound::Bounded {
        max_size: MAX_VALUE_SIZE_BYTES_NODE_PROVIDER_REWARDS,
        is_fixed_size: false,
    };
}
