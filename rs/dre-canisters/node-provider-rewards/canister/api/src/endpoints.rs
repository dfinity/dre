...
impl From<rewards_calculator_results::DayUTC> for DayUTC {
    fn from(value: rewards_calculator_results::DayUTC) -> Self {
        let dd_mm_yyyy = DateTime::from_timestamp(value.get() as i64 / 1_000_000_000, 0)
            .unwrap()
            .naive_utc()
            .format("%d-%m-%Y")
            .to_string();
...
#[derive(candid::CandidType, candid::Deserialize)]
/// see [`rewards_calculator_results::NodeMetricsDailyProcessed`]
