...
use crate::metrics_types::SubnetMetricsKeyStored;
...
async fn subnet_metrics_added_correctly() {
    ...
    for i in 0..days {
        let key = SubnetMetricsKeyStored {
            ts: i * ONE_DAY_NANOS,
            subnet_id: subnet_1,
        };
...
async fn partial_failures_are_handled_correctly() {
    ...
    let key = SubnetMetricsKeyStored { ts: 0, subnet_id: subnet_1 };
    assert!(
        mm.subnets_metrics.borrow().get(&key).is_none(),
        "Metrics should not be present for subnet 1"
    );

    let key = SubnetMetricsKeyStored { ts: 0, subnet_id: subnet_2 };
...
async fn daily_metrics_correct_overlapping_days() {
    ...
    let overlapping_sub_1 = daily_metrics
        .iter()
        .find(|(sub, day, _)| sub == &subnet_1 && day.unix_ts_at_day_end() == DayEnd::from(2 * ONE_DAY_NANOS).get())
    ...
    let overlapping_sub_2 = daily_metrics
        .iter()
        .find(|(sub, day, _)| sub == &subnet_2 && day.unix_ts_at_day_end() == DayEnd::from(2 * ONE_DAY_NANOS).get())
        .map(|(_, _, node_metrics)| node_metrics)
        .unwrap();
