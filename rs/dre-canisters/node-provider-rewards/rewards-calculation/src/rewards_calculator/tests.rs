// use super::*;
// use crate::input_builder::{RewardsCalculatorInputBuilder, SubnetDailyFailureRate};
// use crate::tests::{node_id, subnet_id};
// use crate::types::{DayEndNanos, RewardPeriod, TimestampNanos, NANOS_PER_DAY};
...
//     }
//
//     pub fn build(self) -> BTreeMap<NodeId, Vec<NodeMetricsDaily>> {
//         let mut metrics_by_node = self.nodes_daily_metrics;
...
//
//     let metrics_by_node = FailureRatesBuilder::new()
//         .with_node_metrics_subnet_proposed_failed_blocks(node_0, vec![(0, subnet_id(2), 2, 0)])
//         .with_node_metrics_subnet_proposed_failed_blocks(node_1, vec![(NANOS_PER_DAY, subnet_id(1), 2, 0)])
//         .with_node_metrics_subnet_proposed_failed_blocks(node_2, vec![])
//         .build();
//
//     let input = RewardsCalculatorInput {
//         reward_period: RewardPeriod::new(0, NANOS_PER_DAY).unwrap(),
//         rewards_table: NodeRewardsTable::default(),
//         daily_metrics_by_node: metrics_by_node,
//         daily_subnets_fr: BTreeMap::new(),
//     };
//
...
//         _marker: PhantomData,
//     };
//     let computed: RewardsCalculator<ComputeRelativeFR> = ctx.next().next();
//     let nodes_failure_rates = computed.calculator_results.nodes_results;
//
//     let node_0_fr = nodes_failure_rates.get(&node_0).unwrap().daily_fr.clone();
//
...
//
//     let node_1_fr = nodes_failure_rates.get(&node_1).unwrap().daily_fr.clone();
//
...
//     let node_2_fr = nodes_failure_rates.get(&node_2).unwrap().daily_fr.clone();
//
...
//     let ctx: RewardsCalculator<ComputeExecutionNodesFR> = RewardsCalculator {
...
//     let ctx = ctx.next();
//     let ctx = ctx.next();
//     let ctx: RewardsCalculator<PerformanceMultipliersComputed> = ctx.next();
