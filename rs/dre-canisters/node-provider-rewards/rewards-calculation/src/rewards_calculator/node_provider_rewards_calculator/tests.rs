use crate::rewards_calculator::tests::{node_id, subnet_id};
use crate::rewards_calculator::RewardsCalculator;
use crate::rewards_calculator_results::RewardsCalculatorResults;
use crate::types::{DayEndNanos, NodeFailureRate, NodeMetricsDaily, RewardPeriod, RewardableNode, TimestampNanos, NANOS_PER_DAY};
...
impl FailureRatesBuilder {
    ...
    pub fn with_node_metrics(self, node_id: NodeId, metrics: Vec<(TimestampNanos, SubnetId, u64, u64)>) -> Self {
        ...
        metrics_by_node.insert(
            ...
            metrics
                .into_iter()
                .map(|(ts, subnet_id, proposed, failed)| NodeMetricsDaily::new(ts.into(), subnet_id, proposed, failed))
                .collect(),
        );
    ...
    pub fn build_and_run(self) -> RewardsCalculatorResults {
        ...
        let rewardables = metrics_by_node
            .keys()
        ...
        let calculator = RewardsCalculator::new(reward_period, NodeRewardsTable::default(), metrics_by_node).unwrap();
...
fn test_calculates_node_failure_rates_correctly() {
    let node_0 = node_id(0);
    let node_1 = node_id(1);
    let node_2 = node_id(2);

    let results = FailureRatesBuilder::new()
        .with_node_metrics(node_0, vec![(0, subnet_id(2), 2, 0)])
        .with_node_metrics(node_1, vec![(NANOS_PER_DAY, subnet_id(1), 2, 0)])
        .with_node_metrics(node_2, vec![])
        .build_and_run()
        .nodes_results;

    let node_0_fr = &results.get(&node_0).unwrap().daily_metrics_processed;

    assert_eq!(node_0_fr.len(), 2);
    assert_eq!(node_0_fr[0].failure_rate, NodeFailureRate::DefinedRelative(dec!(0)));

    assert_eq!(node_0_fr[1].failure_rate, NodeFailureRate::Extrapolated(dec!(0)));

    let node_1_fr = &results.get(&node_1).unwrap().daily_metrics_processed;

    assert_eq!(node_1_fr[0].failure_rate, NodeFailureRate::Extrapolated(dec!(0)));
    assert_eq!(node_1_fr[1].failure_rate, NodeFailureRate::DefinedRelative(dec!(0)));

    // Node 2 has no metrics
    let node_2_fr = &results.get(&node_2).unwrap().daily_metrics_processed;

    assert_eq!(node_2_fr[0].failure_rate, NodeFailureRate::Extrapolated(dec!(0)));
}
//
// #[test]
// fn test_node_assigned_same_day_multiple_subnets() {
//     let nodes_failure_rates = FailureRatesBuilder::new()
...
//     let (nodes_failure_rates, _) = FailureRatesBuilder::default().build_and_run();
...
//     let (nodes_failure_rates, subnets_failure_rates) = FailureRatesBuilder::default().build_and_run();
//     let ctx: NodeProviderRewardsCalculator<ComputeExecutionNodesFR> = NodeProviderRewardsCalculator {
...
//     let ctx = ctx.next();
//
//     let extrapolated_failure_rate = ctx.calculator_results.extrapolated_fr;
//
...
//     let ctx = ctx.next();
//     let ctx = ctx.next();
//     let ctx: NodeProviderRewardsCalculator<PerformanceMultipliersComputed> = ctx.next();
//     let performance_multiplier_by_node = ctx.calculator_results.performance_multipliers;
//
