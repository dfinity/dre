...
use crate::{
    ...
    util::{extract_title_and_text, utf8, yesno},
};
...
impl ExecutableCommand for Motion {
    ...
    async fn execute(&self, ctx: crate::ctx::DreContext) -> anyhow::Result<()> {
        ...
        if let HowToProceed::DryRun = ctx.mode {
            println!("Motion that would have been submitted:\n{:#?}", request);
            return Ok(());
        } else if let HowToProceed::Confirm = ctx.mode {
            println!("Motion that will be submitted:\n{:#?}", request);
            if !yesno("Do you want to submit the motion?", true).await?? {
                return Ok(());
            }
        }

        let forum_post = crate::forum::handler(&self.parameters.forum_parameters)?
        ...
        forum_post
            .add_proposal_url(match (response.proposal_id, response.message) {
                (Some(proposal_id), msg) => {
                    info!("Submitted proposal has ID {}", proposal_id.id);
