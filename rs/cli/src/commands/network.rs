    ...
    async fn execute(&self, ctx: crate::ctx::DreContext) -> anyhow::Result<()> {
        ...
        for proposal in proposals {
            if let crate::ic_admin::IcAdminProposalCommand::ChangeSubnetMembership { subnet_id, .. } = &proposal.command {
                ...
                if let Err(e) = ForumEnabledProposalExecutor::from_executor_and_mode(&self.forum_parameters, ctx.mode.clone(), ctx.executor().await?)
                    .propose(proposal.clone(), ForumPostKind::ReplaceNodes { subnet_id: *subnet_id, body })
                    .await
                {
                    errors.push(DetailedError {
