...
impl NodeRewards {
    ...
    fn print_comparison_console(
        ...
        for (provider_id, daily_rewards) in &nrc_data.providers_rewards {
            ...
            let adjusted_total: u64 = daily_rewards.iter().map(|(_, reward)| reward.total_adjusted_rewards_xdr_permyriad.unwrap()).sum();

            // Calculate base rewards total
            let base_total: u64 = daily_rewards.iter().map(|(_, reward)| reward.total_base_rewards_xdr_permyriad.unwrap()).sum();
