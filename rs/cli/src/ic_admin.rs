...
pub struct ProposalResponse(u64);

impl FromStr for ProposalResponse {
    type Err = anyhow::Error;

    fn from_str(response: &str) -> Result<Self, anyhow::Error> {
        ...
        Ok(ProposalResponse(captured))
    }
}

pub trait ProposalId {
    fn id(&self) -> u64;
}

impl ProposalId for ProposalResponse {
    fn id(&self) -> u64 {
        self.0
    }
}
...
/// for ic-admin, and also has the ability to deserialize a proposal
/// response string (from ic-admin output).
/// The type Output is used to set which output type to expect and
/// deserialize when submitting a proposal.
pub trait ProposableViaIcAdmin: Send + Sync {
    type Output: FromStr;

    fn to_ic_admin_arguments(&self) -> anyhow::Result<Vec<String>>;
}

/// Knows how to simulate any ProposableViaIcAdmin, and submit any
/// of them too, returning the deserialized response based on the
/// ProposableViaIcAdmin::Output type.
...
impl ProposalExecutor {
    pub fn from(ic_admin: Arc<dyn IcAdmin>) -> Self {
        Self { ic_admin: ic_admin.clone() }
    }

    pub fn submit<'c, 'd, T: ProposableViaIcAdmin + 'c>(
        &'d self,
        cmd: &'c T,
        forum_post_link: Option<Url>,
    ) -> BoxFuture<'c, anyhow::Result<T::Output>>
    where
        'd: 'c,
        <<T as ProposableViaIcAdmin>::Output as FromStr>::Err: Display,
    {
        Box::pin(async move {
            ...
            let parsed = T::Output::from_str(&res);
            parsed.map_err(|e| anyhow::anyhow!("Failed to deserialize result of proposal execution {}: {}", res, e))
        })
    }

    pub fn simulate<'c, 'd, T: ProposableViaIcAdmin + 'c>(&'d self, cmd: &'c T) -> BoxFuture<'c, anyhow::Result<()>>
    where
        'd: 'c,
    {
        Box::pin(async move {
...
impl ProposableViaIcAdmin for IcAdminProposal {
    type Output = ProposalResponse;
...
"#;
        let parsed = ProposalResponse::from_str(text).unwrap();
