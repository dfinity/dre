...
nodes_dfinity: typing.List[str] = None
out_dir = None
...
def step_1_ssh_readonly_user():
    ...
    run_results = ssh_run.run_on_guests(map(lambda x: x[1], nodes_all), commands, GUEST_USER_READONLY)
...
def step_1_ssh_admin_user():
    ...
    run_results = ssh_run.run_on_guests(map(lambda x: x[1], nodes_external), commands, GUEST_USER_ADMIN)
    ...
    run_results = ssh_run.run_on_guests(map(lambda x: x[1], nodes_dfinity), commands, GUEST_USER_ADMIN)
...
def step_2_versions():
    ...
    run_results = ssh_run.run_on_guests(nodes_ipv6, commands, GUEST_USER_READONLY)
    ...
    for node, node_result in zip(nodes, run_results):
        return_code = node_result[0]
        commit = node_result[1].decode("utf8")
        commit_date_str = ""
        if commit:
            ...
            if commit_date < commit_minimal_date:
                ...
                commit_date_str += " OK"
        stderr = node_result[2].decode("utf8")
...
def step_3_no_state():
    ...
    ssh_run = ic_utils.IcSshRemoteRun(deployment_name, out_dir, node_filter)
    ssh_run.check_run_on_guests(nodes_ipv6, commands, GUEST_USER_READONLY)
