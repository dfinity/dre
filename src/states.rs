...
pub struct NodeReplacementStateDb {
    db: Arc<r2d2::Pool<r2d2_sqlite::SqliteConnectionManager>>,
    not_added: Mutex<Vec<(String, String, String)>>,
    cfg: OperatorConfig,
}

struct NodeReplacementState {
    waiting: String,
    to_remove: String,
    subnet: String,
}

impl NodeReplacementStateDb {
    pub fn new(db: Arc<Pool<SqliteConnectionManager>>, cfg: OperatorConfig) -> Self {
    ...
    pub fn update_proposals(&self) -> Result<(), anyhow::Error> {
        ...
        for proposal in waiting {
            // TODO - actually support multi-threading here and deal with unwraps.
