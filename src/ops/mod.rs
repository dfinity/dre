...
pub async fn get_decentralized_nodes(url: &str, client: &Client, params: DecentralizedNodeQuery) -> Result<BestNodesResponse, anyhow::Error> {
    let resp = client.post(url).json(&params).send().await?.json::<BestNodesResponse>().await?;
    Ok(resp)
}

pub async fn get_dead_nodes(subnet: Subnet, url: &str, client: &Client) -> Result<NodesToRemoveResponse, anyhow::Error> {
    let resp = client.get(url).query(&[("subnet", subnet.id)]).send().await?.json::<NodesToRemoveResponse>().await?;
    Ok(resp)
}
