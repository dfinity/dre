...
class IcDeploymentInventory:
    ...
    def __init__(self, deployment_name, decentralized_deployment=False):
    ...
    def _load_baseline_config(self):
        ...
        # Load a complete list of ipmi macs for each host
        self.ipmi_macs_filename = BASE_DIR / "env/ipmi-macs.yml"
        self.ipmi_macs = yaml.load(open(self.ipmi_macs_filename, encoding="utf8"), Loader=yaml.FullLoader)
        ...
        if "ipv6_address" not in host.vars:
            ...
            if not ipv6:
                ...
                if ic_host:
                    ...
                    if self.decentralized_deployment and phy_fqdn in self.ipmi_macs:
                        mac_address = calculate_deterministic_mac(
                            self.deployment_name, self.ipmi_macs[phy_fqdn], guest_number
                        )
                        ipv6 = calculate_deterministic_ipv6(mac_address, ipv6_prefix)
                    elif self.deployment_name == "staging":
                        mac_address = mac_address_mainnet(
                            phy_system_serial_number, self.deployment_name, host.vars["node_index"]
                        )
...
# so we get deterministic results.
...
def calculate_deterministic_mac(deployment: str, management_nic_mac: str, guest_index: int):
    ...
    mac_lower = management_nic_mac.lower()
...
# TODO the function signature is missing ipv6 prefix argument.
# In case we get a DC with prefix other than /64 the code will break.
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Use decentralized deployment IPv6 generation algorithm.",
    )
    ...
    decentralized_deployment = args.decentralized_deployment or os.getenv("DECENTRALIZED_DEPLOYMENT", "false") == "true"

    deployment_inventory = IcDeploymentInventory(
        deployment_name=deployment_name, decentralized_deployment=decentralized_deployment
    )
