...
class IcDeploymentInventory:
    ...
    def _load_baseline_config(self):
        ...
        self.phy_serial_numbers = yaml.load(
            open(self.phy_serial_numbers_filename, encoding="utf8"),
    ...
    def _host_patch_vars(self, host):
        ...
        if "ipv6_address" not in host.vars:
            ...
            if ipv6:
                ...
                host.vars["ipv6_address"] = str(ipv6) + "/64"
                if not ansible_host:
    ...
    def media_config(self):
        ...
        result = {
            ...
            # TODO inconsistent naming - not sure if this config is used anywhere else
        ...
        for dc_name in sorted(deployment_dcs):
            ...
            for node_name in ic_nodes_by_dc[dc_name]:
                ...
                if "ipv6_interface" in node_vars.keys():
                    node_config["ipv6_interface"] = node_vars["ipv6_interface"]
                ...
                if "ipv4_interface" in node_vars.keys():
                    node_config["ipv4_interface"] = node_vars["ipv4_interface"]
